            ...
            <div className="card carrinho__item" key={`item-carrinho-${index}`}>
              <div className="card-body">
                ...
                <p className="card-text">
                  {`Preço Unidade: R$ ${props.itens[produtoId].preco},00 | Quantidade: ${props.itens[produtoId].quantidade}`}
                </p>
                <p className="card-text">
                  {`Valor: R$
                ${props.itens[produtoId].preco * props.itens[produtoId].quantidade},00`}
                </p>
                <button onClick={props.onRemoveItemCarrinho.bind(null, produtoId)} className="btn btn-danger btn-sm">Remover
                </button>
              </div>  
            </div>
    ...
    return (
      <div className="carrinho">
        <div className="carrinho__itens">
          <CardCarrinhoComponent {...props} />
        </div>
        <div className="carrinho__total mt-2 p-3">
          <h6> Total :
            <strong>{`R$${valorTotal(props.itens)}`}</strong>
          </h6>
        </div>
      </div>
    )
